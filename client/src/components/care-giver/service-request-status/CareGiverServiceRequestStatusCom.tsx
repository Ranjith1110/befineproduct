...
const mockCaregiverRequests = [
    {
        ...
        status: 'completed',
        postServiceNotes: 'Vitals normal. Administered prescribed BP medication.'
    },
    {
        id: 'REQ-1044',
        patient: 'Alice Smith',
...
const CareGiverServiceRequestStatusCom = () => {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'scheduled': return <Badge variant="warning" className="text-[10px] px-2 py-0.5">Scheduled</Badge>;
        ...
        <div className="flex flex-col gap-4 max-w-[1400px] mx-auto w-full relative pb-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                <div>
                    ...
                    <p className="text-xs font-medium text-slate-500 mt-1">
                        View your assigned user requests, track schedules, and log post-service updates.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-xs font-medium text-slate-500">Currently In Progress</div>
                    </div>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-xs font-medium text-slate-500">Completed (MTD)</div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                                ...
                                <tr key={req.id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <Button 
                                                variant={req.status === 'completed' ? 'outline' : 'secondary'} 
                                                size="sm" 
                                                className="h-7 text-[10px] px-2.5"
                                                onClick={() => handleManageRequest(req)}
                                            >
                                                {req.status === 'completed' ? 'View Log' : 'Update Log'}
                                            </Button>
                ...
                <div className="p-3 border-t border-slate-100 flex items-center justify-between text-xs font-medium text-slate-500">
                    <span>Showing 1 to 4 of 61 requests</span>
                ...
                <>
                    ...
                    <div className="fixed inset-y-0 right-0 z-50 w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-300 flex flex-col">
                        <div className="flex items-center justify-between p-5 border-b border-slate-100">
                            <div>
                                <h2 className="text-lg font-bold text-slate-900 tracking-tight">Service Update Log</h2>
                                <p className="text-xs font-medium text-slate-500">Manage status and post-visit notes for {selectedRequest.id}</p>
                            </div>
                            <Button
                                variant="ghost"
                                size="sm"
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    <Select
                                        ...
                                        options={[
                                            ...
                                            { label: 'In Progress (Active Visit)', value: 'in-progress' },
                                            { label: 'Completed', value: 'completed' },
                                        ]}
                                    />
                                </div>
                                {selectedRequest.status === 'scheduled' && (
                                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 mt-1 flex gap-2">
                                        <Activity size={14} className="text-blue-600 shrink-0 mt-0.5" />
                                        <p className="text-[11px] font-medium text-blue-800">
                                            Mark this request as <strong>In Progress</strong> when you arrive at the location and begin the service.
                                        </p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex flex-col gap-1.5 w-full">
                                    ...
                                    <div className="relative">
                                        ...
                                        <textarea
                                            className="w-full bg-slate-50 border border-transparent focus:bg-white focus:border-primary/30 rounded-xl focus:ring-4 focus:ring-primary/10 text-xs font-medium text-slate-800 placeholder-slate-400 p-2.5 pl-9 resize-none h-28"
                                            placeholder="Log vitals, procedures performed, patient response, and any follow-up recommendations..."
                                            defaultValue={selectedRequest.postServiceNotes}
                                            readOnly={selectedRequest.status === 'completed' && selectedRequest.postServiceNotes !== ''}
                                        ></textarea>
                                    </div>
                                </div>
                                ...
                                {selectedRequest.status !== 'completed' && (
                                    <div className="bg-amber-50 p-3 rounded-lg border border-amber-100 flex gap-2">
                                        ...
                                        <p className="text-[11px] font-medium text-amber-800">
                                            Detailed notes are required when marking a visit as completed to ensure accurate patient records.
                                        </p>
                                    </div>
                                )}
                            </div>
...
export default CareGiverServiceRequestStatusCom;