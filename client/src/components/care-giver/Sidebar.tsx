...
const menuItems = [
    { name: 'Dashboard', icon: LayoutDashboard, path: '/care-giver/dashboard' },
    { name: 'My Clients', icon: CalendarDays, path: '/care-giver/my-clients' },
    { name: 'Service Request and Status', icon: Users, path: '/care-giver/service-request-status' },
    { name: 'Appointments', icon: ClipboardCheck, path: '/care-giver/appointments' },
    { name: 'Reports', icon: ClipboardCheck, path: '/care-giver/reports' },
    { name: 'Settings', icon: Settings, path: '/care-giver/settings' }, // <-- Added Settings route
];
...
export function Sidebar({ isOpen, toggleSidebar }: SidebarProps) {
    ...
    const displayName = user ? `${user.firstName}` : 'Caregiver';
    const roleDisplay = user?.roles?.includes('CAREGIVER') ? 'Care Giver' : 'Healthcare Professional';
        ...
        <>
            ...
            >
                ...
                <div className="px-5 py-3 flex items-center gap-3 mb-1">
                    <div className="w-[40px] h-[40px] rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-lg border border-primary/20 shrink-0">
