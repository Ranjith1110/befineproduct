...
const patientOptions = [
    { label: 'All Assigned Patients', value: 'all' },
    { label: 'Eleanor Pena', value: 'pt-1' },
    { label: 'Alice Smith', value: 'pt-3' },
    { label: 'Robert Fox', value: 'pt-4' },
    { label: 'Jane Cooper', value: 'pt-5' }
];

const mockCaregiverReports = [
    {
        id: 'REP-7042',
        patientId: 'pt-1',
    ...
    },
    {
        ...
        title: 'Post-Op Wound Healing Image',
...
const CareGiverReportsCom = () => {
    const [selectedPatient, setSelectedPatient] = useState('all');
    const [isUploadModalOpen, setIsUploadModalOpen] = useState(false);

    const filteredReports = selectedPatient === 'all'
        ? mockCaregiverReports
        : mockCaregiverReports.filter(report => report.patientId === selectedPatient);
    ...
    const getBgForCategory = (category: string) => {
        switch (category) {
            ...
            case 'Observation': return 'bg-amber-50';
        ...
        <div className="flex flex-col gap-4 max-w-[1400px] mx-auto w-full relative pb-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 font-heading tracking-tight mb-0 flex items-center gap-2">
                        Patient Reports & Logs
                    </h1>
                    <p className="text-xs font-medium text-slate-500 mt-1">
                        View clinical history or upload new vitals, charts, and field observations for your patients.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3">
                    ...
                    <div>
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Total Uploads</div>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3">
                    ...
                    <div>
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Submitted This Week</div>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3">
                    ...
                    <div>
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Reviewed by Provider</div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100">
                                ...
                                <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Upload Date & Time</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filteredReports.length > 0 ? filteredReports.map((report) => (
                                <tr key={report.id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="px-4 py-3">
                                        {report.status === 'submitted' && <Badge variant="primary" className="text-[9px] px-2 py-0.5">Submitted</Badge>}
                ...
                <>
                    ...
                    <div className="fixed inset-y-0 right-0 z-50 w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-300 flex flex-col">
                        <div className="flex items-center justify-between p-5 border-b border-slate-100">
                            <div>
                                <h2 className="text-lg font-bold text-slate-900 tracking-tight">Upload Field Report</h2>
                                <p className="text-xs font-medium text-slate-500">Submit vitals, notes, or images to the provider.</p>
                            </div>
                            <Button
                                variant="ghost"
                                size="sm"
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    ...
                                    <Select
                                        ...
                                        options={[
                                            ...
                                            { label: 'Clinical Note / Vitals', value: 'clinical' },
                                            { label: 'Therapy Progress', value: 'therapy' },
                                            { label: 'Observation / Incident', value: 'observation' },
                                            { label: 'Lab Results Upload', value: 'lab' },
                                        ]}
                                    />
                                    <Input
                                        label="Report Title"
                                        placeholder="e.g., Morning Vitals Log"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="border-2 border-dashed border-primary/20 rounded-xl bg-slate-50 hover:bg-primary/5 transition-colors p-6 flex flex-col items-center justify-center gap-3 cursor-pointer">
                                    ...
                                    <div className="text-center">
                                        <p className="text-xs font-bold text-slate-700">Tap to upload or take a photo</p>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    ...
                                    <textarea
                                        ...
                                        placeholder="Add specific observations, concerns, or context regarding this report..."
                                    ></textarea>
                                </div>
                            </div>
                        </div>
...
export default CareGiverReportsCom;