                                ...
                                <tr key={client.id} className="hover:bg-slate-50/50 transition-colors group">
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-3">
                                            <img src={client.avatar} alt={client.name} className="w-8 h-8 rounded-full object-cover border border-slate-200" />
                                            <div>
                                    ...
                                    <td className="px-4 py-3">
                                        ...
                                        {client.status === 'new' && <Badge variant="primary">New</Badge>}
                                    </td>
                                    <td className="px-4 py-3">
                                        {client.caretaker ? (
                                            <div className="flex items-center gap-2">
                                                <div className="w-6 h-6 rounded-full bg-secondary/20 text-primary flex items-center justify-center font-bold text-xs">
                                                    {client.caretaker.charAt(0)}
                                                </div>
                                                <span className="text-xs font-semibold text-slate-700">{client.caretaker}</span>
                                            </div>
                                        ) : (
                                            <Button variant="ghost" size="sm" className="text-primary bg-primary/5 hover:bg-primary/10 h-7 text-xs px-2" leftIcon={<UserPlus size={12} />}>
                                                Assign Caretaker
                                            </Button>
                                        )}
                                    </td>
                ...
                <>
                    ...
                    <div className="fixed inset-y-0 right-0 z-50 w-full md:w-[600px] bg-white shadow-2xl transform transition-transform duration-300 flex flex-col">
                        <div className="flex items-center justify-between p-5 border-b border-slate-100">
                            <div>
                                <h2 className="text-lg font-bold text-slate-900 tracking-tight">Onboard New Client</h2>
                                <p className="text-xs font-medium text-slate-500">Enter primary details to start onboarding.</p>
                            </div>
                            <Button
                                variant="ghost"
                                size="sm"
                        ...
                        <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3">
                            <Button variant="ghost" size="sm" onClick={() => setIsAddClientOpen(false)}>
                                Cancel
                            </Button>
